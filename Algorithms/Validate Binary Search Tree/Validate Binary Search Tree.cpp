...
public:
    bool isValidBST(TreeNode* root) {
        ...
        if(root->right && root->right->val <= root->val)
            return false;
        if(isValidBST(root->left) && isAllSmall(root->left,root->val) && \
           isValidBST(root->right) && isAllLarge(root->right,root->val))
            return true;
        else return false;
    }
    bool isAllSmall(TreeNode* root,int x)
    {
        if(!root)
            return true;
        if(root->val >= x)
            ...
            return isAllSmall(root->left, x) && isAllSmall(root->right, x);
    }
    bool isAllLarge(TreeNode* root,int x)
    {
        if(!root)
            return true;
        if(root->val <= x)
            ...
            return isAllLarge(root->left, x) && isAllLarge(root->right, x);
    }
};
int main()
{
